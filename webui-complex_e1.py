        ...
        if os.path.exists(ANALYSIS_JSON_PATH):
            with open(ANALYSIS_JSON_PATH, 'r', encoding='utf-8') as f:
                ...
                if action_info != last_data:
                    ...
                    lg.debug(action_info)
                    yield action_info  # 返回最新的分析结果

        else:
            yield "<b>Currently, there are no available analysis results. </b>"
...
with gr.Blocks() as demo:
    with gr.Row():
        ...
        with gr.Column(scale=1):  # 设置左侧列比例
            ...
            video_output = gr.Video(label="Video", format="mp4", height=450, width=730)  # 设置视频输出的宽度
